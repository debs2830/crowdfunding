...
$field_name[0] = "eventperson.eventid";
$field_value[0] = $_GET['eventid'];
$query_1=$field_name[0]."=".$field_value[0];
 
$result = mysqli_query($con,"SELECT * FROM eventperson  WHERE ". $query_1 ." ORDER BY lastname");

$query = "SELECT COUNT(*) FROM eventperson  WHERE ". $query_1; 
$resultcount = mysqli_query($con, $query) or die(mysql_error());

$query = "SELECT COUNT(*) FROM eventperson  WHERE CHECKEDIN = 1 AND  ". $query_1; 
...
$.get( "markedcheckin.php", { id:idNum, ifcheck: $.trim($('#checkTD' + idNum).text()), eventid : $('#eventid').val() , ind:1} )
.done(function( data ) {
	...
	<div>
		...
		<table id="datatables" class="display" style="font-size:small">
			...
			<tbody>
				...
				<tr>
<td id="checkTD<? echo $row['id']?>" onclick="sentCheck('<? echo $row['id']?>');return false;" class="underline" data-txid="<? echo $row['tnxid']?>">
                               <?php 
					...
					<td>
					<?php
					/*$date=date_create( $row['dateEntered']);
