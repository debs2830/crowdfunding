...
<title>Edit Reunion Reservation</title>
...
$(window).bind("pageshow", function() {
	...
	<form action="update.php" method="post" id="form" name="form" onsubmit="return validateForm1();">
		...
		<a href="menu.php">Back To Menu </a> | <a href="listEvent.php?eventid=3">Back To Event List </a>
		<table>
...
echo'<input type="hidden" name="tnxid" id="tnxid"  value="'. $row['tnxid'] .'"></input>';
		...
		echo '			<td><label for="city">City</label></td>';
			...
			echo '	id="email" name="email" required="required" /> <br /> ';?>
				<p>	Class of: <input  required type="text" name="amtdescr"  id="amtdescr" value="<?php echo $row['amtdescr'];?>"></p>
<p>	Maiden Name: <input  required type="text" name="notes"  id="notes" value="<?php echo $row['notes'];?>"></p>
...
Amount Attending: <input type="text" name="noadults" id="noadults" value="<?php echo  $row['numadults']; ?>">
...
echo '				  />';
	echo '<br/><label for="totaldue">Paid:</label> ' . $row['payed'] ; 
...
echo '	   <input type="hidden" name="eventid" id="eventid" value="3"/>';
	   
	   ?>
 <input type="hidden" name="description" id="eventid" value="Reunion Reservation" />
	<?php
	if( !isset($_GET['mode']) )
	{
