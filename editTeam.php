...
session_start();
// store session data
$id = (isset ($_GET['eventid']) ? $_GET['eventid'] : 10);

require('connection.php');

$query_1= "SELECT * FROM crowdfundingTeams WHERE ID = " . $_GET['id'];
...
{ ?>
		...
		<div class="form-group">
			...
			<input class="form-control" type="text"
				...
				required="required" value="<?php echo $row['realname'];?>" /> 
			<small>Put just the name. We will add the word TEAM in front. IE:  Fleisher</small>
		</div>
		<div class="form-group">
			<label for="lastname">Nickname:</label>
		<input type="text"
				id="nickname" name="nickname" class="form-control" required value="<?php echo $row['nickname'];?>"/>
		...
		<div class="form-group">
			...
			<input type="number"
				id="donationamt" name="donationamt" class="form-control" required value="<?php echo $row['donationamount'];?>"/>
		</div>	<div class="form-group">
			<label for="cvv">Amount of People Goal: </label>
		
			<input type="number"
				id="peopleGoal" name="peopleGoal" class="form-control" required value="<?php echo $row['peopleGoal'];?>"/>
		</div>
			  	<div class="form-group">
					<button type="submit">SUBMIT</button>
			  </div>
		   		
	   <input type="hidden" name="eventid" id="eventid" value="<?php echo $row['eventid'];?>"></input>
